...
export const ProjectCard = ({ index, hoveredIndex, setHoveredIndex }) => {
    ...
    return (
        <div onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={(() => setHoveredIndex(null))} className={`flex gap-5 p-3 rounded-md bg-white border border-0 border-b-2 duration-300 hover:bg-[#FAFAFC] cursor-pointer hover:-translate-y-px ${(hoveredIndex !== null && index !== hoveredIndex) ? 'blur-[2px]' : ''}`}>
            <img className="max-w-40 rounded-lg" src="https://d38b044pevnwc9.cloudfront.net/cutout-nuxt/enhancer/2.jpg" alt="" />
            <div>
