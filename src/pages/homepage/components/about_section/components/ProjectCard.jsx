...
export const ProjectCard = ({ project, index, hoveredIndex, setHoveredIndex }) => {
    return (
        <a href={project.web_link} onMouseEnter={() => setHoveredIndex(index)} onMouseLeave={(() => setHoveredIndex(null))} className={`flex gap-5 p-3 py-5 rounded-md bg-white border border-0 border-b-2 duration-300 hover:bg-[#FAFAFC] cursor-pointer hover:-translate-y-px ${(hoveredIndex !== null && index !== hoveredIndex) ? 'blur-[2px]' : ''}`}>
            <img className="min-w-40 bg-slate-100 min-h-24 rounded-lg" src={project.thumbnail_url} alt="thumbnail" />
            <div>
